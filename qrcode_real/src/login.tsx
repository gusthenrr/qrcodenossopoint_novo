...
useEffect(() => {
  ...
  (async () => {
    ...
    try {
      const resp = await fetch(`${API_URL}/guardar_login`, {
        method: 'POST',
      ...
      timeoutId = setTimeout(() => {
        navigate(`/cardapio?${KEY}=${number}`, { replace: true });
      }, 2000);
    } catch (e) {
  ...
  const handleLogin = async (e: React.FormEvent) => {
   e.preventDefault();
    ...
    try {
      ...
      if (code){  
      setIsLoading(true);
      const resp = await fetch(`${API_URL}/auth/sms/create`, {
        method: "POST",
        headers: { "Content-Type": "application/json"},
